...
void mergeSequence(struct Sequence sequence, struct Table table,int i,const char* tmp_file )//roznica wzgledem zadania 2
{
    int j=i;
    pid_t mainPid=getpid();
    pid_t childPid=fork();
    pid_t *childProcesses=malloc((sequence.size)*sizeof(pid_t));
    ...
    for (int j;j<=i;j++) wait(childProcesses[j]);
    free(childProcesses);
}
